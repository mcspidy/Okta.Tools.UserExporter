...
namespace Okta.Tools.UserExporter
{
    public class Apps
    {
        ...
        public static async Task<int> AppsMain()
        {
            Common.SetFile(Common.OutputUsersAppsFileName);

            try
            {
                var oktaClient = new OktaClient(Common.OktaApiKey, new Uri(Common.OktaUrl));

                // HttpClient to call /api/v1/users/{id}/appLinks
                using (var http = new HttpClient { BaseAddress = new Uri(Common.OktaUrl), Timeout = TimeSpan.FromSeconds(120) })
                {
                    ...
                    // Prepare CSV header
                    var headerColumns = new[] { "Id", "App Id", "Date Time" };
                    ...
                    using (var sw = new StreamWriter(fs, new UTF8Encoding(false)))
                    {
                        ...
                        // Iterate all users and fetch their apps with throttled HTTP + pagination
                        int userIndex = 0;
                        foreach (var userId in Common.UserIds)
                        {
                            ...
                            // Fetch app links for this user (handles 429/5xx and pagination)
                            var appsArray = await Common.GetAllPagesAsJArrayAsync(http, $"/api/v1/users/{userId}/appLinks?limit=200");

                            foreach (var a in appsArray)
                            {
                                // Okta AppLink includes appInstanceId (application id)
                                var appId = (string)a["appInstanceId"] ?? string.Empty;

                                var fields = new List<string>(2);
                                fields.Add(Common.CsvEscape(userId));
                                fields.Add(Common.CsvEscape(appId));
                ...
                Console.WriteLine("User apps export completed successfully.");
                return 0;
            }
            catch (Exception ex)
            {
